...
var destiny:Vector2 = Vector2(0,0)
...
func _update():
	if entity != null:
		#portrait = entity.get_node("Sprite/SubViewport/Sprite2D").get_texture()#portrait = entity.portrait
	...
	self.destiny.y = (self.size.y)*self.scale.y*(order-1)
